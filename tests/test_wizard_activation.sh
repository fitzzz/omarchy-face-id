...
temporary_dir=$(mktemp -d -t face-unlock-activation.XXXXXX)
trap 'rm -rf -- "$temporary_dir"' EXIT

install -d "$temporary_dir/config" "$temporary_dir/bin"
install -m 0644 "$project_root/packaging/pam/omarchy-lock-face" "$temporary_dir/pam-service"
...
OMARCHY_FACE_UNLOCK_CONFIG_ROOT="$temporary_dir/config" \
OMARCHY_FACE_UNLOCK_PAM_PATH="$temporary_dir/pam-service" \
QT_QPA_PLATFORM=offscreen \
QT_QUICK_BACKEND=software \
    "$app_binary" --integration-install-test

plugin_dir="$temporary_dir/config/omarchy/plugins/fitzzz.face-unlock"
